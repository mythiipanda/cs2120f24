...
then there is at least one counterexample.
...
def p2a  : PLExpr := itsRaining ⇒ sprinklerOn ⇒ p0
#eval! is_unsat p2
/-!
...
-/
def p8  : PLExpr :=
  (itsRaining ∨ sprinklerOn) ⇒ (itsRaining ⇒ streetWet) ⇒ (sprinklerOn ⇒ streetWet) ⇒ streetWet
#eval! is_valid p8
/-!
