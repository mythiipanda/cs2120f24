...
thus need to show is that for all n, if n ∈ ∅ then n ∈ t. To prove that,
we assume n is an arbitrary natural number and assume n ∈ ∅. We then show that n ∈ t. But n ∈ ∅ is a contradiction, so there is no element in the empty set that is in t. Thus, ∅ ∈ 𝒫 t.
-/
    ...
    (
      ...
      Or.elim l
        ...
        (fun mem45 =>
          ...
          | Or.inr five =>
            (by
              exact nomatch (And.right h five)
            ))
        )
    )
  )
...
The proof of this proposition is by case analysis on the proof
of n ∈ (t \ { 5 }). We know that n ∈ (t \ { 5 }) is a proof
of a conjunction, so we can use the And elimination rule to
extract from it a proof of n ∈ t. We then do a case analysis
on this proof, which is a proof of a disjunction. If n = 3,
then n ∈ s. If n = 4 or n = 5, then n ∈ s. We can prove that n = 3 is in s by using the Or introduction rules. We can prove that n = 4 is in s by using the Or introduction rule. However, n = 5 contradicts the assumption that n ∈ (t \ { 5 }), so we can conclude that n ∈ s. This completes the proof.
-/
