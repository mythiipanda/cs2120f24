...
example : nextKloo Marv = Wond := rfl
example : nextKloo Wond = Fabl := rfl
example : nextKloo Fabl = Marv := rfl
...
-/
inductive Product (α β : Type) where
| pair (a : α) (b : β) : Product α β
def swap : Product Bool Nat → Product Nat Bool
| Product.pair a b => Product.pair b a
example : swap (Product.pair true 3) = Product.pair 3 true := rfl
/-!
...
| _, _, Or.inl p => Or.inr p
| _, _, Or.inr q => Or.inl q
...
def pfL : KIFC ∨ CIFC := Or.inl (isFromCville.license Kevin)
def pfR : KIFC ∨ CIFC := Or.inr (isFromCville.license Carter)
