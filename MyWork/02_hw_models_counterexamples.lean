...
/-
You answers here:
Affirm disjunct counter example:
Variables are isRaining, isSprinkling; isRaining is true, is Sprinkling is true
Explanation: Affirm disjunct is a fallacy because if it is both raining and sprinkling, the expression implies that if it is raining, then it is not sprinkling, which is not true, making this expression a fallacy.
-/
...
#eval! interpStringsFromInterps cxs1 2 /-deny_antecedent-/
#eval! interpStringsFromInterps cxs2 2 /-affirm_consequent-/
/-
Your answers here:
Deny antecedent counter example:
Variables are isRaining, isSprinkling; isRaining is true, is Sprinkling is false
Explanation: Deny antecedent says that if (whenever it's sprinkling, then it's raining), then if it is not sprinkling, then it is not raining, which is not true because if its not sprinkling, it can still rain due to some other factor, making this expression a fallacy.

Affirm consequent counter example:
Variables are isRaining, isSprinkling; isRaining is true (it is raining), is Sprinkling is false
Explanation: Affirm consequent says that if (whenever it's sprinkling, then it's raining), then whenever it is raining, then it is sprinkling, which is not true because it can rain without sprinkling, making this expression a fallacy.
-/
